...
- (IBAction)onLoginClick:(id)sender {
    NSString *email = [self findTextField:ViewElement_Username].text;
    NSString *password = [self findTextField:ViewElement_Password].text;
    NSError *error;
    
    NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
    NSURLSession *session = [NSURLSession sessionWithConfiguration:configuration delegate:self delegateQueue:nil];
    NSURL *url = [NSURL URLWithString:@"https://api.voiptella.com/authentication"];
    ...
    [request addValue:@"01EMM1C4M3VRMA63R08NSGFA4D" forHTTPHeaderField:@"x-client"];
    
    [request setHTTPMethod:@"POST"];
    NSDictionary *mapData = [[NSDictionary alloc] initWithObjectsAndKeys: email, @"email",
                             password, @"password", @"mobile",@"device",
                             nil];
    ...
    NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSLog(@"%@",newStr);
        NSError *jsonError;
        NSMutableDictionary  * responseDict= [NSJSONSerialization JSONObjectWithData:data options:NULL error:&jsonError];
        if(jsonError) {
            ...
            [[NSUserDefaults standardUserDefaults]setObject:dataDict[@"app_extension"] forKey:@"username"];
            [[NSUserDefaults standardUserDefaults]setObject:dataDict[@"secret"] forKey:@"password"];
            [[NSUserDefaults standardUserDefaults]setObject:dataDict[@"domain"] forKey:@"domain"];
            NSString * displayName = [NSString stringWithFormat:@"%@ %@", dataDict[@"first_name"],dataDict[@"last_name"]];
            [[NSUserDefaults standardUserDefaults]setObject:displayName forKey:@"dname"];
            dispatch_async(dispatch_get_main_queue(), ^{
                [self onSucessLogin:sender];
            });
...
-(void)onSucessLogin:(id)sender{
    ...
    ONCLICKBUTTON(sender, 100, {
        ...
        linphone_account_params_set_register_enabled(accountParams, TRUE);
        linphone_account_params_set_push_notification_allowed(accountParams, TRUE);
        linphone_account_params_enable_rtp_bundle(accountParams, TRUE);
        linphone_account_params_set_outbound_proxy_enabled(accountParams, FALSE);
        
      /*  LinphoneProxyConfig *defaultCfg = linphone_core_get_default_proxy_config(LC);
        linphone_proxy_config_enable_register(defaultCfg, TRUE);
        linphone_proxy_config_set_push_notification_allowed(defaultCfg, TRUE);*/
       ...
       // linphone_nat_policy_set_stun_server(LNP, [username UTF8String]);
        ...
        linphone_nat_policy_set_stun_server_username(LNP, [tname UTF8String]);
        ...
        linphone_core_set_media_encryption_mandatory(LC, TRUE);
        linphone_core_set_use_rfc2833_for_dtmf(LC, FALSE);
    //    linphone_core_set_video
...
-(void)onSucessLogin1:(id)sender{
    ...
    ONCLICKBUTTON(sender, 100, {
       ...
       /* NSString *domain = [self findTextField:ViewElement_Domain].text;
        ...
        linphone_address_set_port(addr, linphone_address_get_port(tmpAddr));
        linphone_address_set_domain(addr, linphone_address_get_domain(tmpAddr));
     //   linphone_address_set_password(addr, pwd.UTF8String);
        linphone_core_set_media_encryption(LC, LinphoneMediaEncryptionDTLS);
       ...
       // linphone_account_params_set_register_enabled(accountParams, TRUE);
        ...
        linphone_nat_policy_set_stun_server(policy, domain.UTF8String);
